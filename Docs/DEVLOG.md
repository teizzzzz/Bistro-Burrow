...
## 2026-06-13（十三）· 修复"无法启动游戏"（主菜单按钮失灵）

### 1. 现象与排查

主菜单渲染正常、按钮 interactable，但点击"继续营业/新开局"毫无反应。
排查链：控制台 0 错误 → 引擎层 Play 正常（Phase=Menu）→ 模拟 onClick
不进白天 → 直接调 `StartContinueGame(0)` 却正常 → 场上发现**残留的失效
按钮**（监听为空）。

### 2. 根因

编辑器一直处于默认的 **"Play 中重编译并继续"** 模式——开发期间在 Play 中
反复改码编译，**域重载会抹掉所有运行时 AddListener 挂的 onClick 监听**，
旧菜单看着完好实际全是"死按钮"。此前通过注册表写
ScriptCompilationDuringPlay 的尝试从未生效（EditorPrefs 键根本不存在）。

### 3. 修复

`EditorPrefs.SetInt("ScriptCompilationDuringPlay", 1)`（Play 结束后再编译，
本次经 GetInt 回读确认生效）。今后 Play 期间改码不再触发域重载，
死按钮/状态丢失这一整类问题在编辑器内绝迹（打包版本本就不存在此问题）。

### 4. 验证

重启 Play 后用 **EventSystem 指针事件**（与玩家鼠标完全同路径）点击
"继续营业"：按钮数恢复为 1，点击 → Phase=Day 正常进店。零代码改动。

> 工作流规则（追加）：永远不要依赖"Play 中热重载继续跑"——运行时挂的
> 监听与非序列化状态都会丢失；要改码先停 Play。

---

## 2026-06-13（十二）· 基建式信息卡 + 老板亲自掌勺 + 落点投影圈（自主优化轮）

### 1. 本次修改目标

按"基建交互 + Sims + 老爹汉堡店"的定位补交互闭环：
① 点谁看谁——员工/老板/顾客都可点选，左下角实时信息卡；
② 老板亲自掌勺——站到灶台旁开火大提速，且**没雇帮厨也能开火**
（采集员开局的老板可以自己撑厨房，这是"你就是大厨"的核心循环）；
③ 拖拽员工时地面显示落点投影圈（放置反馈）。

### 2. 涉及文件及职责变动

- **`UI/InfoCardUi.cs`**（新增）：独立 Overlay 画布左下小卡，每帧拉取选中
  目标——员工卡：名字+[老板/帮厨/采集员]、勤/耐/日薪、行为一句话
  （MoodText）、疲劳条（绿→红随派遣上限）；顾客卡：点单与菜价、
  行为一句话、等餐时显示耐心条；目标销毁（顾客离店）自动隐藏。
- **`Bistro/BistroDirector.cs`**：`_bossAgent` + `BossChefActive`
  （|老板x−灶台x|<1.7 且未被拎起）；AutoCookTick 改为
  "有帮厨 或 老板掌勺"即可开火，掌勺时间隔 ×0.55；灶台上方
  "老板掌勺中·火力全开！"提示牌随状态开关。
- **`Bistro/StaffAgent.cs`**：`Stats` 暴露 + `MoodText()`（按当前动画态映射：
  Sleep=呼呼大睡 / Move=巡场 / Interact=掌勺 / 被拎起 / 听候差遣）。
- **`Bistro/CustomerAgent.cs`**：点选碰撞体 + `MoodText()`（九段状态文案）。
- **`Bistro/BistroCameraController.cs`**：射线分流升级（员工→点选/长按提起；
  顾客→仅点选）；选中即弹信息卡并镜头跟随；拖拽时绿色落点投影圈
  （压扁 RadialGlow 跟随落点 x/z，松手销毁）。

### 3. 验证记录（Unity MCP Play 实测）

| 流程 | 结果 |
|---|---|
| 无帮厨掌勺 | 新档（采集员老板团团，hasCook=False）：老板站灶台 → BossChefActive=true、提示牌亮、成功做菜上菜 served=1（"你就是大厨"闭环成立） |
| 信息卡·员工 | 团团[老板] 勤4耐4日薪14 +"听候差遣"+ 疲劳条（截图 infocard_boss.png 四要素同框） |
| 信息卡·顾客 | 选中食客显示"吃得正香 / 翠叶精灵沙拉"；离店后卡片自动隐藏 |
| 回归 | 编译 0 错误；EditMode **20/20** |

> 已知边界（仅脚本可触发）：同一帧内 回主菜单→继续 会撞上 BistroScene
> 异步卸载未完成（CreateScene 同名异常）；真实 UI 跨帧操作不会出现，暂不修。
