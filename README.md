...
**玩法**（与 Unity 端一致）：
- **白天（60 现实秒 = 游戏内 08:00–18:00）**：顾客排队入座点单，**点击头顶气泡开火做菜**，上菜越快小费越多；
- **黄昏结算**：对账 → 菜品研发（风味矩阵融合）→ 店外采购（吸引力引擎）→ 雇佣/派遣员工 → 战前用餐（以吃代练 Def）；
- **夜晚（78 现实秒）**：横版探索，`A/D` 移动、`空格` 跳、`J/点击` 攻击、门口 `E` 收队。背包越重饱食度掉越快，饿到 0 持续掉血，昏厥丢一半战利品；
- **黎明**：派遣结算、疲劳恢复、自动存档（浏览器 localStorage / Unity PlayerPrefs→IndexedDB）。
